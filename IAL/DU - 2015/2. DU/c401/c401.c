...
**/
	if(RootPtr != NULL)
	{
		if(RootPtr->Key == K) //Klíč nalezen
		{
            *Content = RootPtr->BSTNodeCont; //Do proměnné uložíme data uzlu
            return TRUE; //a vrátímě TRUE
        }

        else //Klíč zatím nenalezen
        {
            if(RootPtr->Key > K) //Hledáme na levé straně
            {
                return BSTSearch(RootPtr->LPtr, K, Content);
            }

            else //Hledáme na pravé straně
            {
    ...
    else //Uzel je prázdný, vratíme FALSE
        return FALSE;
...
**/
    ...
    if(*RootPtr == NULL)
    {
        tBSTNodePtr novy = (tBSTNodePtr)malloc(sizeof(struct tBSTNode)); //Alokujeme paměť

        if(novy == NULL) //Máme dostatek místa?
            return;

        novy->Key = K; //Do nového uzlu vložíme klíč
        novy->BSTNodeCont = Content; //Do nového uzlu vložíme hodnotu
        novy->LPtr = NULL;
        novy->RPtr = NULL;
        *RootPtr = novy; //Uložíme nový uzel
    }
...
**/
    ...
    if(*RootPtr != NULL)
    {
        if((*RootPtr)->Key == K) //Klíč nalezen
        {
            if((*RootPtr)->LPtr == NULL) //Uzel nemá levý podstrom
            {
                pom = *RootPtr;
            ...
            else if((*RootPtr)->RPtr == NULL) //Uzel nemá pravý podstrom
            {
                pom = *RootPtr;
                *RootPtr = (*RootPtr)->LPtr;
                free(pom);
            ...
            else //Uzel má oba podstromy
                ReplaceByRightmost(*RootPtr, &(*RootPtr)->LPtr);
        }

        else if((*RootPtr)->Key > K) //Hledáme na levé straně
        {
            BSTDelete(&(*RootPtr)->LPtr, K);
        }

        else if((*RootPtr)->Key < K) //Hledáme na pravé straně
        {
...
**/
    ...
    {
        int levaVyska = BSTHeight(NodePtr->LPtr, TRUE);
        int pravaVyska = BSTHeight(NodePtr->RPtr, TRUE);

        if(levaVyska > pravaVyska)
            return(levaVyska + 1);
        else
            return(pravaVyska + 1);
    }
