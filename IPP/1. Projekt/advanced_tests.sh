...
$INTERPRETER $TASK.$EXTENSION --input=${LOCAL_IN_PATH}test01.jsn --output=${LOCAL_OUT_PATH}test01.xml -r="koren" 2> ${LOG_PATH}test01.err
echo -n $? > test01.!!!
...
$INTERPRETER $TASK.$EXTENSION  -n --types --input=${LOCAL_IN_PATH}test03.jsn -r="koren" > "${LOCAL_OUT_PATH3}test03.xml" 2> ${LOG_PATH}test03.err
echo -n $? > test03.!!!
...
$INTERPRETER $TASK.$EXTENSION --input=${LOCAL_IN_PATH}test06.jsn --output=${LOCAL_OUT_PATH}test06.xml -l -r="koren" 2> ${LOG_PATH}test06.err
echo -n $? > test06.!!!
...
$INTERPRETER $TASK.$EXTENSION --input=${LOCAL_IN_PATH2}test09.jsn --output=${LOCAL_OUT_PATH}test09.xml -n -s -r="koren" 2> ${LOG_PATH}test09.err
echo -n $? > test09.!!!
...
$INTERPRETER $TASK.$EXTENSION --input="${LOCAL_IN_PATH3}test15.jsn" --output=${LOCAL_OUT_PATH2}test15.xml --array-name="b<a>d" -r="koren" 2> ${LOG_PATH}test15.err
echo -n $? > test15.!!!

# test16: -i a dale -h=xyz, takze skript neskonci chybou; Expected output: test16.xml; Expected return code: 0
$INTERPRETER $TASK.$EXTENSION -i --input="${LOCAL_IN_PATH3}test16.jsn" --output="${LOCAL_OUT_PATH3}test16.xml" -h=xyz -r="koren" 2> ${LOG_PATH}test16.err
echo -n $? > test16.!!!
...
do
  if [ $i -le 9 ]
  then
    FIRST_OUT="${LOCAL_IN_PATH}test0${i}.!!!"
    SECOND_OUT="${REF_OUT_PATH}test0${i}.!!!"
    FIRST_XML="${LOCAL_IN_PATH}test0${i}.xml"
    SECOND_XML="${REF_OUT_PATH}test0${i}.xml"
    
  else
    FIRST_OUT="${LOCAL_IN_PATH}test${i}.!!!"
    SECOND_OUT="${REF_OUT_PATH}test${i}.!!!"
    FIRST_XML="${LOCAL_IN_PATH}test${i}.xml"
    SECOND_XML="${REF_OUT_PATH}test${i}.xml"
  fi
  
  
  CHANGE_OUT=$(diff $FIRST_OUT $SECOND_OUT)
  CHANGE_XML=$(java -jar /pub/courses/ipp/jexamxml/jexamxml.jar $FIRST_XML $SECOND_XML delta.xml jsn_options)
  CHANGE_XML=$(echo $?)
  value=$(cat $FIRST_OUT)
  ...
  then
    ...
    then
      echo "${CHANGE_XML}"
      echo "XML soubory pri testu ${i} se lisi!"
      err_xml=1
    fi
  fi
...
if [ $err_xml -eq 0 ]
then
  echo "Vsechny XML soubory jsou stejne."
fi   
