 ...
 * dátum: 6.5.2017
 * <p>
 * Trieda implementujúca metódy pre hranie hry.
 */
                ...
                if (fromCard.isTurnedFaceUp()) {
                    for (CardStack s : tableau) {                       // prejdenie tableau
                        if (s.checkPut(fromCard)) {
                            toCard = s.get();
                            text = ("Move " + fromCard.toString() + " to " + toCard.toString() + ".");
                            return text;
                        }
                    }
                    for (CardDeck d : foundations) {                    // prejdenie foundations
                        if (d.checkPut(fromCard)) {
                            //toCard = d.get();
                            text = ("Move " + fromCard.toString() + " to " +/* toCard.toString() +*/ "foundation.");
                            return text;
                        }
                    }
                }
            ...
            for (CardStack st : tableau) {
                if (st.checkPut(fromCard)) {
                    ...
                    if (toCard == null && fromCard.value() == 13)
                        ...
                        text = ("Move card from waste to " + toCard.toString() + ".");
                    return text;
                }
            }
            for (CardDeck deck : foundations) {
                if (deck.checkPut(fromCard)) {
                    text = ("Move card from waste to foundation");
                    return text;
                }
            }
        }
